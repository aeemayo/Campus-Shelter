...
import { fetchProperty, adminApproveProperty, adminDeleteProperty } from "@/services/properties";
...
export default function AdminPropertyReview() {
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      toast({ title: "Property Approved", description: "The listing is now live on the platform." });
      refetch();
    } catch {
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      toast({ title: "Property Rejected", description: "The listing has been rejected." });
      refetch();
    } catch {
  ...
  const statusBadge = property.status === "APPROVED" || property.approved
    ? <Badge variant="success" className="text-sm px-3 py-1">Approved</Badge>
    : property.status === "REJECTED"
    ? <Badge variant="destructive" className="text-sm px-3 py-1">Rejected</Badge>
      ...
      <main className="flex-1 pt-24 pb-12">
        <div className="max-w-6xl mx-auto px-4">
          ...
          <div className="flex items-center gap-1.5 text-sm text-muted-foreground mb-6">
            <Button variant="ghost" size="sm" className="h-7 px-2 gap-1.5" onClick={() => navigate("/admin")}>
          ...
          <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 mb-8">
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                Submitted {new Date(raw.createdAt).toLocaleDateString("en-NG", { year: "numeric", month: "long", day: "numeric" })}
                {raw.updatedAt !== raw.createdAt && (
                  <> · Updated {new Date(raw.updatedAt).toLocaleDateString("en-NG", { year: "numeric", month: "long", day: "numeric" })}</>
                )}
              </p>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-6">
                      ...
                      {property.images.length > 1 && (
                        <div className="grid grid-cols-4 gap-2">
                          {property.images.slice(1, 5).map((img, i) => (
                            ...
                            >
                              <img src={img} alt={`Photo ${i + 2}`} className="w-full h-full object-cover transition-transform group-hover:scale-105" />
              ...
              <Card className="border-border/60">
                ...
                <CardContent>
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                    {[
                      { label: "Furnished", value: raw.furnished, icon: <Sofa className="w-4 h-4" /> },
                      { label: "Wi-Fi", value: raw.wifi, icon: <Wifi className="w-4 h-4" /> },
                      { label: "Electricity Backup", value: raw.electricityBackup, icon: <Zap className="w-4 h-4" /> },
                      { label: "Water Supply", value: raw.water, icon: <Droplets className="w-4 h-4" /> },
                      { label: "Security", value: raw.security, icon: <ShieldCheck className="w-4 h-4" /> },
                    ].map((item) => (
                      <div
                        key={item.label}
                        className={cn(
              ...
              {/* Inspection Slots */}
              {property.inspectionSlots && property.inspectionSlots.length > 0 && (
                <Card className="border-border/60">
                  <CardHeader className="pb-3">
                    <CardTitle className="text-base flex items-center gap-2">
                  ...
                  <CardContent>
                    <div className="flex flex-wrap gap-2">
                      {property.inspectionSlots.map((slot, i) => (
                        <Badge key={i} variant="secondary" className="text-xs px-3 py-1.5 font-medium">
            ...
            <div className="space-y-6">
              ...
              <Card className="border-border/60">
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="space-y-3">
                    {[
                      { icon: <Home className="w-4 h-4" />, label: "Type", value: roomTypeLabels[property.type] || property.type },
                      { icon: <Bed className="w-4 h-4" />, label: "Rooms", value: raw.rooms },
                      { icon: <Bath className="w-4 h-4" />, label: "Bathrooms", value: raw.bathrooms },
                      { icon: <MapPin className="w-4 h-4" />, label: "Distance", value: property.distance },
                      { icon: <Calendar className="w-4 h-4" />, label: "Available From", value: raw.availableFrom ? new Date(raw.availableFrom).toLocaleDateString("en-NG", { year: "numeric", month: "short", day: "numeric" }) : "Not set" },
                      { icon: <Star className="w-4 h-4" />, label: "Rating", value: property.rating > 0 ? `${property.rating} (${property.reviewCount} reviews)` : "No reviews yet" },
                    ].map((item) => (
                  ...
                  {raw.latitude && raw.longitude && (
                    <div className="pt-1">
                      ...
                      <p className="text-xs font-mono text-foreground">{raw.latitude.toFixed(6)}, {raw.longitude.toFixed(6)}</p>
                    </div>
                  )}
                </CardContent>
              </Card>
              ...
              <Card className="border-border/60">
                ...
                <CardContent className="space-y-2 text-xs text-muted-foreground">
                  <div className="flex justify-between">
                    ...
                    <span className="font-mono text-foreground truncate ml-2 max-w-[160px]">{raw.id}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Created</span>
                    <span className="text-foreground">{new Date(raw.createdAt).toLocaleString("en-NG")}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Updated</span>
                    <span className="text-foreground">{new Date(raw.updatedAt).toLocaleString("en-NG")}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Bookings</span>
                    <span className="text-foreground">{raw._count?.bookings ?? 0}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Reviews</span>
                    <span className="text-foreground">{raw._count?.reviews ?? 0}</span>
                  </div>
                </CardContent>
              </Card>
            </div>
          ...
          <div className="flex-1 relative flex items-center justify-center p-6 pt-20">
            ...
            <div className="absolute inset-0 flex items-center justify-between px-4 pointer-events-none">
              <Button
                ...
                onClick={(e) => { e.stopPropagation(); setLightboxIndex((p) => p > 0 ? p - 1 : property.images.length - 1); }}
              >
              ...
              <Button
                ...
                size="icon"
                className="w-10 h-10 rounded-full bg-white/10 text-white pointer-events-auto hover:bg-white/20"
                onClick={(e) => { e.stopPropagation(); setLightboxIndex((p) => p < property.images.length - 1 ? p + 1 : 0); }}
              >
